...
If *dataset* is not provided, the data will be generated using *schema*
 ...
 - **device** – encapsulates specific set of values and target system properties. 
	 - Devices can be used to override specific parameters of *processing rules*. This feature can be used when you'd like to encapsulate such parameters as *id*, *geo position* and inject them to a payload. 
...
Replaying dataset without modifying any parameters is not really useful use case, so let's **replay dataset with updated date/timestamp properties**. Go to *create session* screen, then:
   ...
   1. Upload dataset which has at least one date/timestamp property on *create definition* screen and apply *schema* derived for it. Dataset may look like the following:
...
If your dataset is correct, on this step you'll see that *Current time* rule is selected by default for the timestamp property. It means that timestamp property will be updated with current time, but the difference between timestamps of two adjacent records will stay the same. Also, if there are more than one timestamp properties in one record, the first one will have *Current time* rule by default, others - *Relative time* rule, which means that timestamps will be updated relatively to *Current time* timestamp or any other timestamp property (if selected).
